...
class ServiceCeremonyJasousai(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = 'service_ceremony_jasousai.py'.replace('/', '_').replace('.py', '')
    self.from_main = from_main
    self.session = HTMLSession()
    self.content = list()

    start = time.time()
    url = "https://ja-sousai.jp/area/"
    self.get_page(url)
    end = time.time()
  ...
  def get_page(self, url):
    ...
    tel_no_list = []
    prefs =soup.select('.pref_box_list a') #get all stores url from soup

    for pref in prefs:
      print(pref)
      try:
        pref_url = 'https://ja-sousai.jp'+pref['href']
      except:
         continue
      page = self.get_response(pref_url)
      soup = BeautifulSoup(page.html.html,'html.parser')

      stores = soup.select('.hall_info_box.bg_300')
      for store in stores:
        
        # self.content.append(self.get_data(store))
        tel = store.find('th',text='電話番号').find_next('a').text
        if tel in tel_no_list:
           continue
        tel_no_list.append(tel)
        print(total)
        total += 1
  ...
  def get_data(self, store):
    ...
    _store['chain_name'] = "JA葬祭"
    _store['CSAR_Category'] = "ss"
    _store['chain_id'] = "service/ceremony/jasousai"
    _store['e_chain'] = "JA Sousai"
