...
class ServiceCeremonyFamillekazokusou(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = 'service_ceremony_famillekazokusou.py'.replace('/', '_').replace('.py', '')
    self.from_main = from_main
    self.session = HTMLSession()
    self.content = list()

    start = time.time()
    url = "https://www.famille-kazokusou.com/search/"
    self.get_page(url)
    end = time.time()
  ...
  def get_page(self, url):
    ...
    areas = soup.select('.search_city .outline_btn a')
    for area in areas:
      area_url = 'https://www.famille-kazokusou.com'+area['href']
      page = self.get_response(area_url)
      soup = BeautifulSoup(page.html.html,'html.parser')
      # municipals = soup.select('.section_part .first a')
      stores = soup.select('.hall_list a')
      # for mun in municipals:
      #   mun_url = mun['href']
      #   page = self.get_response(mun_url)
      #   soup = BeautifulSoup(page.html.html,'html.parser')
      # stores =soup.select('.search_list_box a[href*="area"]') #get all stores url from soup
      # print(mun_url)
      for store in stores:
        # if store.select_one('img[src*="noimage"]') == None:
  ...
  def get_data(self, store):
    ...
    _store['chain_name'] = "ファミーユ"
    _store['CSAR_Category'] = "ss"
    _store['chain_id'] = "service/ceremony/famillekazokusou"
    _store['e_chain'] = "Famille"
    _store['categories'] = "ceremony"
    _store['業種大'] = "サービス"
    _store['業種中'] = "セレモニーホール"
  ...
  def get_response(self, url,trying = 0):
    if 'http' not in url:
       url = 'https://www.famille-kazokusou.com'+url
...
if __name__ == '__main__':
  ServiceCeremonyFamillekazokusou(True)
